...
function (_Component) {
  ...
  _createClass(Coins, [{
    ...
    value: function render() {
      ...
      var coinsData = filterCoin.filter(function (coin, index, arra) {
        arra.map(function (item, pos) {
          console.log(arra.indexOf(item[pos]), filterCoin.indexOf(coin[index]));

          if (item.id === coin.id && arra.indexOf(item) === filterCoin.indexOf(coin)) {} else if (item.id === coin.id && arra.indexOf(item[pos]) !== filterCoin.indexOf(coin[id])) {
            arra.splice(arra.indexOf(item), 1);
          }
        });
      }); // return (
      //   <div className="col-xl-4 col-md-4 col-sm-4 col-4 text-center mt-5">
...
//# sourceMappingURL=5.1da82fef0c60f37fe6d0.hot-update.js.map